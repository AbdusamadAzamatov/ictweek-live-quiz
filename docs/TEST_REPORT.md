...
| `@ictquiz/server` | 9 | 53 | login / authz isolation / CSRF origin; **change-password** (happy path revokes other sessions and flips `mustChangePassword`, wrong current → 401, short new → 400, unauthenticated → 401); quiz CRUD, export→import, organizer scoping; session creation (PIN, snapshot, media URLs); media upload magic-byte + size checks; CSV preview/import; **live engine**: full game, idempotent submissions (incl. retry after close and retry while pending), stale / late / malformed rejection, answer-key never sent to player/display before reveal, lock / remove / token kill, resume + duplicate tabs, late-join eligibility, restart → RECOVERY → REPLAY without double scoring, per-socket rate limit, diagnostics; **content slides + polls**: CONTENT_SLIDE transitions, poll scoring skip, poll reveal without leaderboard, stable CONTENT_SLIDE boot recovery; **scoring oracle**: independent inline recomputation of every submission's points/correctness and the final ranking; **burst**: 150 same-tick answers, close racing a burst, batch failure → TEMPORARY rollback; **reports**: standings/questions/responses from durable rows, CSV formula escaping, end-mid-question voids the attempt |
